...
import { EditProducts } from "../lib/fetching/product.lib";
...
export default function EditData({open, handleOpen, namaProp, deskripsiProp, hargaProp, stokProp, idProp }: EditDataProps, n) {
  const [supplier, setSupplier] = useState([])
  const [nama, setNama] = useState(namaProp)
  ...
  useEffect(() => {
    setNama(namaProp)
    setDeskripsi(deskripsiProp)
    setHarga(hargaProp)
    setStok(stokProp)
  }, [namaProp, deskripsiProp, hargaProp, stokProp])
  ...
  const handleTambahData = async () => {
    const response = await EditProducts(idProp,nama, deskripsi, harga, stok, foto, idSupplier)
  ...
  return (
    <Dialog open={open} handler={handleOpen}>
      <DialogHeader>Edit Data Produk</DialogHeader>
      <div className="px-4">
        <div>
